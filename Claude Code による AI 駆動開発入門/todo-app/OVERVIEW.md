...
| `src/todo_app/todo.clj` | `todo-app.todo` | ドメインロジック（タスク操作・ステータス定義） |
...
```
validate-input   入力の検証（エラーチェック・パース）
     ↓
...
JAR 実行時は `todo_app/store.clj` リソースの URL プロトコルが `jar:` かどうかで判定し、JAR の絶対パスを `java.net.URI` 経由で取得する（URL エンコード対策）。
