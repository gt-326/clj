...
└── chap21/
    ├── common/
    │   ├── layer1/
    │   │   ├── core.clj             # Proc レコード定義
    │   │   └── stat.clj             # グローバル状態・スケジューラ（PROC / PROCS / pick-process）
    │   └── layer2/
    │       ├── controller.clj       # プロセス制御（fork / wait / yield / halt / kill）
    │       └── generator.clj        # プログラムマクロ（program / program-cps）
    ├── black_board.clj              # ブラックボード（claim / unclaim / check）
    ├── warring_state_period.clj     # 戦国時代ドメイン関数（my-take / fortify / loot / ransom）
...
│   ├── common/
│   │   ├── layer1/
│   │   │   ├── core_test.clj        # Proc レコードのテスト
│   │   │   └── stat_test.clj        # グローバル状態・スケジューラのテスト
│   │   └── layer2/
│   │       └── controller_test.clj  # プロセス制御のテスト
...
## コアプロセス管理 — `common/layer1/core.clj` / `common/layer1/stat.clj`
...
## プロセス制御 — `common/layer2/controller.clj`
...
## プログラムマクロ — `common/layer2/generator.clj`
...
`fork` で登録したプロセスを `pick-process` で順次実行し、`halt` が呼ばれると例外で停止する。
...
(defn claim   [& f] (when (seq f) (swap! BBOARD conj f)))           ; アサートを追加
(defn unclaim [& f] (when (seq f) (swap! BBOARD #(remove #{f} %))))  ; アサートを削除
(defn check   [& f] (when (seq f) (seq (filter #(= % f) @BBOARD))))  ; アサートを検索
```
...
`claim` / `unclaim` / `check` はいずれも引数なし（`(seq f)` が falsy）のとき何もしない。
...
(defn capture [city]
  (w/my-take city)
  (c/setpri 1)       ; 優先度を 1000 → 1 に下げて yield
  (c/yield (w/fortify city)))
...
|---|---|---|
| `common/layer1/core_test.clj` | `make-proc` | 1 / 3 |
| `common/layer1/stat_test.clj` | `multiple-value-bind` / `most-urgent-process` / `pick-process` | 7 / 25 |
| `common/layer2/controller_test.clj` | `fork` / `halt` / `setpri` / `kill` / `arbitrator` / `wait` / `yield` | 9 / 21 |
...
### `common/layer1/core_test.clj` — テスト内容

| テスト名 | 検証内容 |
|---|---|
| `make-proc-test` | フィールド設定・`:wait` 省略時の nil・Proc レコード型 |

### `common/layer1/stat_test.clj` — テスト内容
...
### `common/layer2/controller_test.clj` — テスト内容
...
テストでは state 関数の末尾で `(con/halt)` を呼ぶことで ExceptionInfo を発生させ停止する。
...
| 末尾再帰 | CL の TCO により自然に末尾最適化 | `(s/pick-process)` で開始し、内部で相互再帰 |
...
Clojure: (defn ~name [~@args] (try (reset! PROCS nil) ~@body (s/pick-process) ...))
```

`=defn` を使った版（`program-cps`）は `generator.clj` / `cps.clj` に比較用として残している。
