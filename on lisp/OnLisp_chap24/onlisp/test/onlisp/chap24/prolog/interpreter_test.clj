...
(defn setup-facts []
  (reset! interp/*rlist* nil)
  (interp/<- (painter canale antonio venetian))
  (interp/<- (painter hogarth william english))
  (interp/<- (painter reynolds joshua english))
  (interp/<- (dates canale 1697 1768))
  (interp/<- (dates hogarth 1697 1772))
  (interp/<- (dates reynolds 1723 1792)))
...
(deftest with-inference-simple-test
  (testing "変数なし：hogarth にマッチ → 1件"
    (let [n (atom 0)]
      (interp/with-inference (painter hogarth william english)
        (swap! n inc))
      (is (= 1 @n))))

  (testing "変数あり：英国人画家の名前（挿入順）"
    (let [results (atom [])]
      (interp/with-inference (painter ?x _ english)
        (swap! results conj ?x))
      (is (= '[hogarth reynolds] @results))))

  (testing "変数あり：全画家の名前・姓・国籍"
    (let [results (atom [])]
      (interp/with-inference (painter ?x ?y ?z)
  ...
  (testing "マッチなし → 結果なし"
    (let [results (atom [])]
      (interp/with-inference (painter nobody ?x ?y)
        (swap! results conj ?x))
      (is (= [] @results)))))
...
(deftest with-inference-and-test
  (testing "painter と dates の結合（英国人のみ）"
    (let [results (atom [])]
      (interp/with-inference (and (painter ?x ?y english)
                                  (dates ?x ?b ?d))
  ...
  (testing "全画家の生没年"
    (let [results (atom [])]
      (interp/with-inference (and (painter ?x _ _)
                                  (dates ?x ?b ?d))
...
(deftest with-inference-or-test
  (testing "or: 英国人またはヴェネチア人（全3画家）"
    ...
    (let [results (atom [])]
      (interp/with-inference (or (painter ?x _ english)
  ...
  (testing "and + or: 英国人で生年が1697か1723"
    (let [results (atom [])]
      (interp/with-inference (and (painter ?x _ english)
                                  (or (dates ?x 1697 _)
                                      (dates ?x 1723 _)))
        (swap! results conj ?x))
      (is (= '[hogarth reynolds] @results)))))
...
(deftest with-inference-rule-test
  (testing "単体ルール：英国人画家"
    ...
    (interp/<- (english-painter ?x) (painter ?x _ english))
    (let [results (atom [])]
      (interp/with-inference (english-painter ?x)
        (swap! results conj ?x))
      (is (= '[hogarth reynolds] @results))))
  ...
  (testing "複合ルール：生年が同じ画家のペア"
    ...
    (interp/<- (same-birth ?x ?y)
               (dates ?x ?b _)
               (dates ?y ?b _))
    (let [results (atom [])]
      (interp/with-inference (same-birth canale ?y)
        (swap! results conj ?y))
